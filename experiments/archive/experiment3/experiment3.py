...
# path to result file
result_file_path = os.path.join(output_dir, f"experiment3_results_{run_id}.pickle")
...
problem = {
    "num_vars": 3,
    "names": ["attack_reward", "discount_factor", "softargmax_coef"],
    "bounds": [[-0.2, 0.1], [0.5, 0.7], [0.01, 1]],
}
...
# there are a total of 2 * 256 * (3 + 2) = 2560 possible samples.
# to run a saltelli sample with 256 samples, run through parameter combinations 0..2559
# to run a saltelli sample with 128 samples, run through parameter combinations 0..1279

# choose appropriate parameters for this run
all_params = [
    (reasoning_level, *params) for params in samples for reasoning_level in (0, 1)
]
reasoning_level, attack_reward, discount_factor, softargmax_coef = all_params[run_id]
...
run_description = f"run {run_id} (level={reasoning_level}, reward={attack_reward}, disc_coef={discount_factor}, softargmax_coef={softargmax_coef})"
