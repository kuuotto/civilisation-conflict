### EXPERIMENT 2 AS (with joint Activation Schedule)
import argparse
import datetime
import os
import pickle
...
# path to result file
result_file_path = os.path.join(output_dir, f"experiment2AS_results_{run_id}.pickle")
...
all_params = [
    (seed, exploration_coef, softargmax_coef)
    for seed in range(n_repetitions)
    for exploration_coef in (0.3, 0.6)
    for softargmax_coef in (0.1, 1)
]
seed, exploration_coef, softargmax_coef = all_params[run_id]
...
params = {
    ...
    "n_tree_simulations": 100 if test_run else 8000,
    "obs_noise_sd": 0.15,
...
run_description = f"run {run_id} (seed={seed}, exploration_coef={exploration_coef}, softargmax_coef={softargmax_coef})"
