...
class Civilisation(mesa.Agent):
    ...
    def _init_action_set(self):
        self._action_set = tuple(ag for ag in self.model.agents 
                                 if ag != self) + (action.HIDE, action.NO_ACTION)
    ...
    def possible_actions(self, state = None):
        ...
        return ((action.NO_ACTION, action.HIDE) + 
                self.model.get_agent_neighbours(agent=self, 
                                                radius=agent_influence_rad))

    def step_tech_level(self):
        """Update own tech level"""
        new_tech_level = growth.tech_level(state=self.get_state(), 
                                           model=self.model)
