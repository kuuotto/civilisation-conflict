...
if TYPE_CHECKING:
    ...
    AgentAction = Union[action.NO_TURN, action.NO_ACTION, action.HIDE, 
...
def joint_to_agent_action_history(joint_action_history: JointActionHistory,
    ...
    """
    return tuple(act[agent] if agent in act else action.NO_TURN 
                 for act in joint_action_history)
...
def weight_particles(particles: Tuple[Particle, ...],
    ...
    """
    # calculate weights 
    weights = tuple(p.previous_particle.weight * 
                    ipomdp.prob_observation(observation=observation,
                                            state=p.state,
                                            action=p.joint_action_history[-1],
                                            agent=agent,
                                            model=model) for p in particles)
...
class BeliefForest:
    ...
    def update_beliefs(self, 
        ...
        weight_particles(new_root_node.particles + reinvig_particles,
        ...
        if sum(p.weight for p in self.top_level_tree_root_node.belief) == 0:
            print("The weights of the particles in the top-level tree root node are all 0")

        # remove references to previous particles
        for particle in new_root_node.particles:
            particle.previous_particle = None

        print(f"{self.top_level_tree.signature}:",
...
class Tree:
    ...
    def get_node(self, agent_action_history: AgentActionHistory) -> Node:
        ...
        for root_node in self.root_nodes:
            ...
            if (agent_action_history[:root_length] == 
    ...
    def expand(self):
        ...
        if len(self.root_nodes) > 1:
            root_node_weights = tuple(n.root_node_weight 
                                      for n in self.root_nodes)
            node = model.random.choices(self.root_nodes, 
                                        weights=root_node_weights)[0]
        else:
        ...
        belief_par = model.random.choices(
            node.belief, 
            weights=tuple(p.weight for p in node.belief))[0]
    ...
    def simulate_from(self, particle: Particle,
        ...
        if not action_unvisited:
            for tree in self.forest.child_trees(parent_tree=self, 
                                                include_parent=True):
    ...
    def update_beliefs(self):
        ...
        for root_node in root_nodes:
            ...
            particles = model.random.choices(node_compatible_particles,
            ...
            for particle in particles:
                ...
                # weight particles
                weight_particles(particles=particles_to_weight,
                                 observation=obs,
                                 agent=self.agent,
                                 model=model)
                ...
                # sample particle
                particle = model.random.choices(particles_to_weight,
            ...
            if n_samples == 0:
                print("No successful samples, pruning root node", 
                      root_node.agent_action_history)
                continue
            else:
    ...
    def tree_policy(self, node: Node, 
                    explore: bool, 
                    softargmax: bool = False) -> AgentAction:
        """
        ...
        W_a = tuple(sum(p.weight for p in node.particles 
        ...
        if explore and 0 in W_a:
            ...
            unexpanded_actions = tuple(action 
        ...
        Q = tuple(sum(p.weight * p.value 
                      for p in node.particles 
                      if p.next_agent_action == action) / weight
                  for action, weight in zip(actions, W_a))
    
        if explore:
            # add exploration bonuses
            Q = tuple(q + model.exploration_coef * np.sqrt(np.log(N) / w_tilde)
                      for w_tilde, q in zip(W_tilde, Q))

        if not explore and softargmax:
            # use softargmax to calculate weights of different actions
            action_weights = tuple(np.exp(w_tilde / np.sqrt(N)) 
                                   for w_tilde in W_tilde)
            action_weight_sum = sum(action_weights)
            action_weights = tuple(w / action_weight_sum 
...
class Node:
    ...
    def weight_particles(self, observation: Observation):
        ...
        """
        weight_particles(particles=self.particles,
                         observation=observation,
    ...
    def generate_reinvigorated_particles(self) -> List[Particle]:
        ...
        n_samples = round(self.root_node_weight * 
                          model.n_reinvigoration_particles)
        ...
        particles = model.random.choices(
            tuple(p for p in self.parent_node.belief if p.weight > 0), 
            k=n_samples)
        ...
        if last_agent_action == action.NO_TURN:
            ...
            # choose an action
            actor_actions = tuple(ipomdp.level0_opponent_policy(agent=actor, 
                                                                model=model)
                                  for actor in actors)
            actions = ({actor: actor_action} 
        ...
        for particle, action_ in zip(particles, actions):
            ...
            # create new particle
            new_state = ipomdp.transition(state=particle.state,
                                          action_=action_,
                                          model=model)
            new_joint_action_history = particle.joint_action_history + (action_,)
            new_particle = Particle(state=new_state,
                                    joint_action_history=new_joint_action_history,
                                    node=self,
                                    previous_particle=particle)
